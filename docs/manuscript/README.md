...
> **Generated + link note (2026-08-31).** The `0X_*.md` section files in this
> folder are GENERATED by `scripts/compose_manuscript.py` (from
> `sheaf/fragments.yaml`) — do not hand-edit them. Their `../output/figures/*.png`
> relative links intentionally resolve only in the composed/rendered context, not
> from the committed tree; fix the generator, not the generated files (see TODO.md
> re-doc-1). The `<id>.png` links in this README and AGENTS.md are schema
> placeholders, not real file references.
