# AGENTS: Results fragments

- Edit the per-track `*.md` fragments here; never edit the composed `docs/manuscript/03_results.md`.
- Each fragment maps to a `(section=results, track)` row in `docs/manuscript/sheaf/manifest.yaml`;
...
- Hold the strict claim ceiling: results are finite software/simulation outputs only; no
  empirical, clinical, neural, ontological, awakening, or practice-efficacy claims.
