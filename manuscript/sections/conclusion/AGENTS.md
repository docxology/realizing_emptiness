...
- Edit the per-track `*.md` fragment here; never edit the composed `manuscript/05_conclusion.md`.
- Each fragment maps to a `(section=conclusion, track)` row in
  `manuscript/sheaf/manifest.yaml`; filenames are the track ids
  (`paper_source`, `qfep`, `evidence_ceiling`, `limitations`).
- Subsection heading and anchor come from `manuscript/sheaf/tracks.yaml` (`section_titles`,
